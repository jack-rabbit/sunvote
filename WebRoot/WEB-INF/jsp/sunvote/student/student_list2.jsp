...
<body class="no-skin">
								...
								<c:when test="${not empty varList}">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.SEX == 1 ? "男":var.SEX == 0 ?"女":"未知"}</td>
											<td class='center'>${var.GROUPID}</td>
											<td class='center'>${var.SIGN_NO}</td>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>student/goAdd2.do?class_id=${pd.CLASS_ID}&term_id=${pd.TERM_ID}';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>student/goEdit2.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		function report(){
			window.open('<%=basePath%>report/report.do?classid=${pd.CLASS_ID}');
		}
