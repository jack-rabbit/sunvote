...
public class KeypadController extends BaseController {
	...
	public ModelAndView list(Page page) throws Exception{
		...
		page.setPd(pd);
		List<PageData>	varList = keypadService.list(page);	//列出Keypad列表
	...
	public ModelAndView exportExcel() throws Exception{
		...
		List<PageData> varOList = keypadService.listAll(pd);
		List<PageData> varList = new ArrayList<PageData>();
		for(int i=0;i<varOList.size();i++){
			PageData vpd = new PageData();
			vpd.put("var1", varOList.get(i).getString("NO"));	    //1
			vpd.put("var2", varOList.get(i).getString("SN"));	    //2
			vpd.put("var3", varOList.get(i).get("IS_OK").toString());	//3
			vpd.put("var4", varOList.get(i).get("WORK_MODE").toString());	//4
			vpd.put("var5", varOList.get(i).getString("PRODUCT_DATE"));	    //5
			varList.add(vpd);
		}
